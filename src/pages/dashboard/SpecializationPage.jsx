...
export default function SpecializationPage() {
  ...
  const handleSubmitCreate = async (data) => {
    if(TOKEN && isValidToken(TOKEN)) {
      const response = await createSpecialization(data, { headers: { authorization: `Bearer ${TOKEN}` } })
  ...
  const handleSubmitUpdate = async (data) => {
    if(TOKEN && isValidToken(TOKEN)) {
      const response = await updateSpecialization(data._id, data, { headers: { authorization: `Bearer ${TOKEN}` } })
      return { ...response.data, code: response.status}
    }
    return { message: "TOKEN_REQUIRED", status: false }
  }
  ...
  const handleSubmitDelete = async (id) => {
    if(TOKEN && isValidToken(TOKEN)) {
      const response = await deleteSpecialization(id, { headers: { authorization: `Bearer ${TOKEN}` } })
      return { ...response.data, code: response.status}
    }
    return { message: "TOKEN_REQUIRED", status: false }
  }
  ...
  return (
    <>
      <Helmet>
        <title>Peminatan Jurusan | Ruang Baca TI UHO</title>
      </Helmet>

      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading="Peminatan Jurusan"
          links={[
            {
              name: 'Beranda',
      ...
      <FormDataDialog 
        title={`${titleDialog} Peminatan Jurusan`}
