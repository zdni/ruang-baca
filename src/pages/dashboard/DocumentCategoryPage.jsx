...
export default function DocumentCategoryPage() {
  ...
  const handleSubmitCreate = async (data) => {
    if(TOKEN && isValidToken(TOKEN)) {
      const response = await createDocumentCategory(data, { headers: { authorization: `Bearer ${TOKEN}` } })
  ...
  const handleSubmitUpdate = async (data) => {
    if(TOKEN && isValidToken(TOKEN)) {
      const response = await updateDocumentCategory(data._id, data, { headers: { authorization: `Bearer ${TOKEN}` } })
      return { ...response.data, code: response.status}
    }
    return { message: "TOKEN_REQUIRED", status: false }
  }
  ...
  const handleSubmitDelete = async (id) => {
    if(TOKEN && isValidToken(TOKEN)) {
      const response = await deleteDocumentCategory(id, { headers: { authorization: `Bearer ${TOKEN}` } })
      return { ...response.data, code: response.status}
    }
    return { message: "TOKEN_REQUIRED", status: false }
  }
  ...
  return (
    <>
      <Helmet>
        <title>Kategori Dokumen | Ruang Baca TI UHO</title>
      </Helmet>

      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading="Kategori Dokumen"
          links={[
            {
              name: 'Beranda',
      ...
      <FormDataDialog 
        title={`${titleDialog} Kategori Dokumen`}
